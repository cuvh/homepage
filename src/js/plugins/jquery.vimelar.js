...
;(function ($, window) {
    ...
    var vimelar = function(node, options) { // should be called on the wrapper div
        ...
        $('<iframe />', {
            ...
            allowfullscreen: 1
        }).prependTo('body').wrap('<div id="vimelar-container" style="overflow:hidden;position:absolute;z-index:17;width:100%;height:100%;background:#00c092;"></div>').after('<div id="vimelar-overlay" style="width:100%;height:100%;z-index:2;position:absolute;left:0;top:0;"></div>');
    ...
    $.fn.vimelar = function (options) {
        return this.each(function () {
            if (!$.data(this, 'vimelar_instantiated')) { // let's only run one
                $.data(this, 'vimelar_instantiated', vimelar(this, options));
            }
        });
    }
